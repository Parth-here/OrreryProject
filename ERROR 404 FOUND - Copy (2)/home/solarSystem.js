...
sunTextureLoader.load("sun.jpg", (texture) => {
    ...
    const glowGeometry = new THREE.SphereGeometry(30, 300, 32); // Increased size for glow
    const glowMaterial = new THREE.MeshBasicMaterial({ col, transparent: true, opacity: 0.5 }); // Changed opacity for better effect
    const glow = new THREE.Mesh(glowGeometry, glowMaterial);
    scene.add(glow);
});

// Function to create rings for Uranus
function createRings(radius) {
    const ringGeometry = new THREE.RingGeometry(radius, radius + 1.75, 32);
    ...
    ring.rotation.x = Math.PI / 4; // Rotate 90 degrees around X-axis
    return ring;
}
...
const planets = [];
const planetData = [
    { name: "Mercury", radius: 30, texture: "VENUS.jpg", orbitSpeed: 0.009 },
    { name: "Venus", radius: 40, texture: "MERCURY.jpg", orbitSpeed: 0.00791 },
    { name: "Earth", radius: 50, texture: "earth.jpg", orbitSpeed: 0.007 },
    { name: "Mars", radius: 60, texture: "https://cdn.pixabay.com/photo/2016/01/20/20/23/mars-1143030_1280.jpg", orbitSpeed: 0.007 },
    { name: "Jupiter", radius: 55, texture: "mars.jpg", orbitSpeed: 0.005 },
    { name: "Saturn", radius: 70, texture: "jupiter3.jpg", orbitSpeed: 0.004 },
    { name: "Uranus", radius: 80, texture: "saturn3.jpg", orbitSpeed: 0.005 },
...
planetData.forEach(data => {
    textureLoader.load(data.texture, (texture) => {
        const geometry = new THREE.SphereGeometry(3.7, 32, 32);
        const material = new THREE.MeshStandardMaterial({ map: texture });
        const planet = new THREE.Mesh(geometry, material);
        planets.push({ mesh: planet, radius: data.radius, orbitSpeed: data.orbitSpeed, angle: 0 });

        // Check if the planet is Uranus and add rings
        if (data.name === "Uranus") {
            const rings = createRings(5); // Adjust the radius of the rings
            planet.add(rings); // Add rings as a child of the planet
        }
